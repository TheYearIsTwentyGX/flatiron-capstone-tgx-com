...
Facility.all.each do |f|
  FacilityAccess.create(user_id: "1", facility_id: f.id, profile: 1)
  @access = FacilityAccess.create(user_id: "2", facility_id: f.id, profile: 1)
end
...
i.times do
  while !@access.new_record? || i == 25
    @access = FacilityAccess.find_or_initialize_by(user_id: User.where("id > ?", 2).all.sample.id, facility_id: Facility.all.sample.id)
